...
import net.trenterprises.diamondcore.cross.api.JavaPlugin;
import net.trenterprises.diamondcore.cross.api.PluginSession;
import net.trenterprises.diamondcore.cross.api.event.Event;
import net.trenterprises.diamondcore.cross.api.event.EventHandler;
	...
	public static void throwEvent(Event e) throws InstantiationException, IllegalAccessException, MalformedURLException, SecurityException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException {
		for(int i = 0; i < Listeners.size();) {
			...
			while(!ran) {
				...
				try {
					PluginSession session = PluginSession.sessionList.get(k);
					...
					/*Ignore, might happen*/
					k++;
					if(k >= PluginSession.sessionList.size()) ran = true;
				}
			}
			i++;
		}
